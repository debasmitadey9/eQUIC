...
def request_ca_values(index, table_id=None, mode="ENC"):
    ca_ip = 'localhost'       # CA server IP
    ca_port = 65433           # CA server port

    context = ssl.create_default_context(ssl.Purpose.SERVER_AUTH)

    context.check_hostname = False
    context.verify_mode = ssl.CERT_NONE  # Disable this in production!

    context.load_verify_locations(cafile='certs/ca_cert.pem')  # Trust CA certificate

    if mode == "DEC":
        context.load_cert_chain(certfile='certs/server_cert.pem', keyfile='certs/server_key.pem')

    with socket.create_connection((ca_ip, ca_port)) as sock:

        with context.wrap_socket(sock, server_hostname='localhost') as tls_sock:
            if mode == "ENC":
                request_message = f"ENC,{index}"
            elif mode == "DEC":
                request_message = f"DEC,{index},{table_id}"
            else:
                raise ValueError("Unknown mode. Use 'ENC' or 'DEC'.")

            tls_sock.sendall(request_message.encode())
            data = tls_sock.recv(1024).decode().strip()

            if "Invalid" in data or "Error" in data:
                print(f"CA Server responded with an error: {data}")
                raise ValueError(f"CA Server Error: {data}")

            try:
                if mode == "ENC":
                    X, X_inv, P, table_id = map(int, data.split(','))
                    return X, X_inv, P, table_id
                else:
                    X, X_inv, P = map(int, data.split(','))
                    return X, X_inv, P
            except ValueError as e:
                print(f"Failed to parse response from CA: {data}")
                raise ValueError(f"Invalid response from CA: {data}") from e
...
class RDTSC:
    ...
    def get_cycles(self):
        ...
        self.libc.clock_gettime(4, ctypes.byref(cycles))
        return cycles.value
...
# @profile
def encrypt_cid(cid):
    cid_int = int.from_bytes(cid, byteorder='big')
    cid_first_2_bits = (cid_int >> (len(cid) * 8 - 2)) & 0b11
    cid_last_2_bits = cid_int & 0b11

    index = (cid_first_2_bits << 2) | cid_last_2_bits
    X, _, P, table_id = request_ca_values(index, None, "ENC")

    encrypted_cid_int = (cid_int * X) % P
    
    encrypted_cid = encrypted_cid_int.to_bytes((encrypted_cid_int.bit_length() + 7) // 8, byteorder='big')

    
    metadata = ((index << 1) | table_id).to_bytes(1, byteorder='big')
    
    encrypted_cid = encrypted_cid + metadata

    return encrypted_cid

# @profile
def decrypt_cid(encrypted_cid_with_metadata):
    
    encrypted_cid = encrypted_cid_with_metadata[:-1]  
    metadata = encrypted_cid_with_metadata[-1] 

    index = (metadata >> 1) & 0b1111
    table_id = metadata & 0b1 

    encrypted_cid_int = int.from_bytes(encrypted_cid, byteorder='big')

    _, X_inv, P = request_ca_values(index, table_id, "DEC")
    
    decrypted_cid_int = (encrypted_cid_int * X_inv) % P

    decrypted_cid = decrypted_cid_int.to_bytes((decrypted_cid_int.bit_length() + 7) // 8, byteorder='big')
...
class QuicConnection:
    ...
    def __init__(
        ...
        *,
        ...
        if not peer_meta:
            ...
            new_cid = encrypt_cid(os.urandom(8))
            peer_meta['cid_queue'].put(new_cid)
        ...
        if self._is_client:
            ...
            self._original_destination_connection_id = (
                decrypt_cid(original_destination_connection_id)
    ...
    def change_connection_id(self) -> None:
        ...
        if self._peer_cid_available:
            # retire previous CID
            self._retire_peer_cid(encrypt_cid(self._peer_cid.cid))
    ...
    def connect(self, addr: NetworkAddress, now: float) -> None:
        ...
        self._start_memory = memory_usage()
        #edited
    ...
    def _parse_transport_parameters(
        ...
        if not from_session_ticket:
            ...
            if self._is_client and (
                ...
                != decrypt_cid(self._original_destination_connection_id)
            ):
                raise QuicConnectionError(
                    error_code=QuicErrorCode.TRANSPORT_PARAMETER_ERROR,
                    frame_type=QuicFrameType.CRYPTO,
    ...
    def _write_application(
        ...
        while True:
            ...
            if self._handshake_complete:
                ...
                # logger.info(f"Handshake completed in {elapsed_time:.2f}s")
                # logger.info(f"Memory used: {memory_diff:.2f} MB")
                #edited
                # ACK
